...
export default class TabsExampleSwipeable extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.purchaseInfo !== this.props.purchaseInfo) {
      ...
      if (nextProps.purchaseInfo.downPayment !== "Cash" && nextProps.purchaseInfo.mortgage !== undefined) {
    ...
    return (
      <div>
        <Tabs onChange={this.handleChange} value={this.state.slideIndex} >
          <Tab label="Conservative" value={0} style={{ backgroundColor: '#191a1c' }} />
          <Tab label="Moderate" value={1} style={{ backgroundColor: '#191a1c' }} />
          <Tab label="Agressive" value={2} style={{ backgroundColor: '#191a1c' }} />
        </Tabs>
